...
With a background in big data in Microsoft Azure, splittable and colum oriented file formats compatible with distributed map/reduce processing (ala Hadoop) are potentially an important consideration.   From this standpoint, [Apache Avro](http://avro.apache.org/) and [Apache Parquet](http://parquet.apache.org/) should be considered as well.   IIX should include design features to address the needs ranging from the smallest of projects to big data, though it's not clear if columinar storage really is an information *exchange* priority.   Instead, I expect to focus more on ensuring that append-only operations are well supported in the streaming interchange format.   Columnizing and indexing representation for query processing is probably outside the use case.   
